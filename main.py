...
def calculate_intervals(dialogue, total_time):
  ...
  min_visible_time = 5
  extra_time_per_prompt = (total_time * 60 -
                           (min_visible_time * num_prompts)) / num_prompts
...
class Teleprompter(tk.Tk):
  ...
  def __init__(self, dialogue, total_time, previous_input):
    ...
    self.paused = False
    self.bind("<Escape>",
              self.exit_to_start)  # Bind Escape to exit to start page
    ...
    self.bind(
        "<space>",
        lambda event: self.toggle_pause())  # Bind spacebar to pause/resume

    global min_visible_time, extra_time_per_prompt
    min_visible_time, extra_time_per_prompt = calculate_intervals(
        dialogue, total_time)
  ...
  def update_text(self, speaker, line):
    self.label.config(text=f"{speaker}: {line}",
                      fg=self.colors.get(speaker,
  ...
  def update_time_left(self):
    ...
    self.time_left_label.config(
        text=f"Time left: {minutes_left}m {seconds_left}s")
...
class StartPage(tk.Tk):
  ...
  def __init__(self, previous_input=""):
    ...
    self.label = tk.Label(
        self,
        text="Enter the dialogue for the teleprompter\n(Speaker: Dialogue)",
        font=("Helvetica", 16),
        bg="#f0f0f0")
...
Speaker 1: Once you enter the details, just hit submit and the script will begin.
Speaker 2: Perfect. Let's get started!"""
    self.text_area.insert(
        "1.0", previous_input or placeholder_dialogue
    )  # Insert the previous input or placeholder dialogue

    self.duration_label = tk.Label(
        self,
        text="Enter total duration of the video in minutes:",
        font=("Helvetica", 12),
        bg="#f0f0f0")
    ...
    self.text_area.bind("<Control-Return>",
                        self.ctrl_enter_submit)  # Bind Ctrl+Enter to submit

    self.submit_button = tk.Button(self,
                                   text="Submit",
                                   command=self.start_teleprompter,
                                   bg="#f0f0f0",
                                   font=("Helvetica", 12))
  ...
  def start_teleprompter(self):
    ...
    try:
      ...
      if dialogue and total_time > 0:
        self.destroy()
        app = Teleprompter(dialogue,
                           total_time,
                           previous_input=dialogue_str.strip())
        app.mainloop()
      else:
        self.label.config(
            text=
            "Invalid dialogue format or duration. Please ensure each line is 'Speaker: Dialogue' and duration is correct."
        )
    except ValueError:
      self.label.config(
          text=
          "Invalid duration. Please enter a numeric value for the duration.")
  ...
  def update_geometry(self):
    ...
    self.after(
        100,
        self.update_geometry)  # Continuously update the size of the window
